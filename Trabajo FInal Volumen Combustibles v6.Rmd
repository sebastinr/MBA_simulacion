...
En nuestro caso el mejór método es el ARIMA.

-	Analizando los distintos métodos mediante KPI (RMSE, MSE, MAPE) etc y compararlos. En particular podemos usar el RMSE (Error Cuadrático Medio). El que nos de la menor medida de RMSE es el método predictivo que más se acercará a futuro a los datos reales para ese periodo de tiempo y dentro de una distribución normal.
En R se puede hacer con “accuracy(vector datos,..),…
Lo que nos devolverá unas columnas con los distintos valores de los indicadores con los cuales comparar los métodos de pronóstico. El que tenga menor valor relativo, mejor es.
