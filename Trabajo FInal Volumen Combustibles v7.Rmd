...
Dado la tendencia y afectación del consumo medido en volumen del año 2020 por la cuarentena, ¿Es posible predecir dicho consumo en los próximos 24 meses?. Tomando el consumo histórico desde el año 2013 a la fecha de los productos: Nafta Grado 3, Nafta Grado 2 y GNC tratamos de predecir aplicando diferentes métodos de pronóstico estadístico los próximos años. Se analizan las características y resultados de dichos análisis.
...
m1_g2 <- snaive(data_serie_super, h=24)
...
```{r}
# elaborando el pronostico
m3_g2 <- hw(data_serie_super, h=96, seasonal = 'multiplicative')
...
# elaborando el pronostico
m4_g2 <- forecast(modelo_arima, h=96)
...
```{r}
# elaborando el método
m1_gnc <- snaive(data_serie_gnc, h=24)
...
# elaborando el pronostico
m2_gnc <- forecast(regresion, h=96)
...
```{r}
# elaborando el pronostico
m3_gnc <- hw(data_serie_gnc, h=96, seasonal = 'multiplicative')
...
# elaborando el pronostico
m4_gnc <- forecast(modelo_arima, h=96)
...
```{r}
# elaborando el método
m1_g3 <- snaive(data_serie_premium, h=24)
...
```{r}
# elaborando el pronostico
m3_g3 <- hw(data_serie_premium, h=96, seasonal = 'multiplicative')
...
# elaborando el pronostico
m4_g3 <- forecast(modelo_arima, h=96)
...
De acuerdo al análisis de los métodos de estudio predictivo de la demanda, podemos inferir que si se pueden predecir a futuro la demanda de consumo de los distintos productos. Cabe aclarar que se debe seleccionar el método más adecuado con el que debemos trabajar en función del error que nos da en la predicción al comparar, la data real con la predicción antes realizada.

Esto podemos hacerlo de dos maneras:
-	Analizando gráficamente su distribución y ajuste( residuales), el método que tenga los residuales mas cercanos a una distribución normal y que su ajuste esté dentro de los parámetros punteados, mejor es.


## Ajuste y verificación del residual del Método Naive de predicción para N.Grado 2 
```{r}
# Verificando el ajuste del método
autoplot(m1_g2)+autolayer(fitted(m1_g2), series="Ajuste")

# verificando los residuales  
checkresiduals(m1_g2)
```

##  Ajuste y verificación del residual del Método regresión con N. Grado 2

```{r}
# verificando el ajuste del método
autoplot(m2_g2)+autolayer(fitted(m2_g2), series="Ajuste")

# verificando los residuales
checkresiduals(m2_g2)
```


## Ajuste y verificación del residual del Método Holt-Winters con N. Grado 2
```{r}
# verificando el ajuste del método
autoplot(m3_g2)+autolayer(fitted(m3_g2), series="Ajuste")

# verificando los residuales
checkresiduals(m3_g2)
```

## Ajuste y verificación del residual del ARIMA N. Grado 2

```{r}

# verificando el ajuste del método
autoplot(m4_g2)+autolayer(fitted(m4_g2), series="Ajuste")

# verificando los residuales
checkresiduals(m4_g2)

```

##  Ajuste y verificación del residual del Métodos Naive GNC

```{r}

# verificando el ajuste del método
autoplot(m1_gnc)+autolayer(fitted(m1_gnc), series="Ajuste")

# verificando los residuales
checkresiduals(m1_gnc)
```

##  Ajuste y verificación del residual del Método regresión GNC


```{r}

# verificando el ajuste del método
autoplot(m2_gnc)+autolayer(fitted(m2_gnc), series="Ajuste")

# verificando los residuales
checkresiduals(m2_gnc)
```


##  Ajuste y verificación del residual del Método Holt-Winters GNC
```{r}

# verificando el ajuste del método
autoplot(m3_gnc)+autolayer(fitted(m3_gnc), series="Ajuste")

# verificando los residuales
checkresiduals(m3_gnc)
```

##  Ajuste y verificación del residual del ARIMA GNC

```{r}

# verificando el ajuste del método
autoplot(m4_gnc)+autolayer(fitted(m4_gnc), series="Ajuste")

# verificando los residuales
checkresiduals(m4_gnc)

```
##  Ajuste y verificación del residual del Método Naive N. Grado 3

```{r}

# verificando el ajuste del método
autoplot(m1_g3)+autolayer(fitted(m1_g3), series="Ajuste")

# verificando los residuales
checkresiduals(m1_g3)
```

##  Ajuste y verificación del residual del Método regresión  N. Grado 3


```{r}

# verificando el ajuste del método
autoplot(m2_g3)+autolayer(fitted(m2_g3), series="Ajuste")

# verificando los residuales
checkresiduals(m2_g3)
```


##  Ajuste y verificación del residual del Método Holt-Winters N. Grado 3
```{r}

# verificando el ajuste del método
autoplot(m3_g3)+autolayer(fitted(m3_g3), series="Ajuste")

# verificando los residuales
checkresiduals(m3_g3)
```

##  Ajuste y verificación del residual del ARIMA  N. Grado 3

```{r}

# verificando el ajuste del método
autoplot(m4_g3)+autolayer(fitted(m4_g3), series="Ajuste")

# verificando los residuales
checkresiduals(m4_g3)
...
El método que tenga los residuales mas cercanos a una distribución normal y que su ajuste esté dentro de los parámetros punteados, mejor es.

En nuestro caso los dos mejores métodos son ARIMA y Holt-Winters como se puede apreciar en los gáficos precedentes.

La prueba final para evaluar estos métodos se realiza con los datos Reales que sucedan en el futuro y se comparan con los estmados por el pronóstico, esto se realiza evaluando diferentes variables dentro de las cuales la más importante es RMSE (también se calculan MSE y MAPE). El que nos de la menor medida de RMSE es el método predictivo que más se acercará a futuro a los datos reales para ese periodo de tiempo y dentro de una distribución normal.
En R esta comparación entre los datos reales y el pronóstico se puede hacer con la función “accuracy(vector datos,..)",Lo que nos devolverá las variables antes mencionadas dentro de las cuales, el método que menor valor de RMSE tenga, mejor es.
